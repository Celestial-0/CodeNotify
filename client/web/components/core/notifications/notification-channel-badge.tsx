...
import { NotificationChannel, NOTIFICATION_CHANNEL_LABELS } from '@/lib/types/notification.types';
...
const statusConfig = {
  sent: { label: 'Sent', dotColor: 'bg-blue-500' },
...
export function NotificationChannelBadge({
  ...
  const badge = (
    ...
    >
      <span className={cn(sizeClass.icon, config.color)}>{config.icon}</span>
      {showLabel && <span className={cn(sizeClass.text, config.color)}>{label}</span>}
      {status && (
        <span
          className={cn('rounded-full', sizeClass.dot, statusConfig[status].dotColor)}
...
// Multiple channel badges component
interface NotificationChannelBadgesProps {
  channels: NotificationChannel[];
  statuses?: Partial<Record<NotificationChannel, 'sent' | 'pending' | 'failed' | 'delivered'>>;
  showLabels?: boolean;
  size?: 'sm' | 'md' | 'lg';
  className?: string;
}
